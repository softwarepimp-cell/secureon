...
<div class="min-h-screen flex items-center justify-center">
  <div class="bg-white border border-slate-200 rounded-xl p-8 w-full max-w-md shadow-sm">
    ...
    <h1 class="text-2xl font-display font-semibold">Create account</h1>
    <?php if (!empty($error)): ?>
      <div class="mt-4 text-red-500"><?= App\Core\Helpers::e($error) ?></div>
    <?php endif; ?>
    <form method="post" class="mt-6 space-y-4">
      <?= App\Core\Helpers::csrfField() ?>
      <input name="name" type="text" placeholder="Name" class="w-full px-4 py-2 rounded bg-white border border-slate-300">
      ...
      <button class="w-full bg-brand text-white px-4 py-2 rounded font-semibold">Create</button>
    </form>
    <div class="mt-4 text-slate-500 text-sm">
      Already have an account? <a href="<?= App\Core\Helpers::baseUrl('/login') ?>">Sign in</a>
    </div>
  </div>
</div>
